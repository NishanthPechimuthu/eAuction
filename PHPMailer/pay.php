...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate inputs
    if (empty($cardholder_name) || empty($cardNumber) || empty($expiryMonth) || empty($expiryYear) || empty($cvv)) {
        ob_clean();
    ...
    $new_entry = [
        ...
        'user_id' => $user_id,
        'username' => $username, // Store userName instead of user_type
    ...
    if ($success !== false) {
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td style="border-bottom: 1px solid #222">Monthly Subscription Fee (' . htmlspecialchars($username) . ')</td>
        ...
        <p><strong>Note:</strong> 90% (₹900) of this amount will be refunded after you complete one selling and one buying transaction without any reports.</p>
        ...
        try {
            ...
            <body>
                <div class="container">
                    ...
                    <div class="content">
                        <p>Dear ' . htmlspecialchars($user["userFirstName"] . " " . $user["userLastName"]) . ',</p>
                        <p>Your monthly subscription fee has been processed successfully.</p>
                        <div class="details">
                            <p><strong>Transaction ID:</strong> ' . htmlspecialchars(explode('_', explode('.', $transaction_tracking_id)[0])[1]) . '</p>
                            <p><strong>Username:</strong> ' . htmlspecialchars($username) . '</p>
                            <p><strong>Card Number:</strong> ' . htmlspecialchars($cardNumber) . '</p>
                            <p><strong>Amount:</strong> ₹' . $subscription_fee . '</p>
                            <p><strong>Description:</strong> Monthly Subscription Fee (' . htmlspecialchars($username) . ')</p>
                        </div>
                        <p><strong>Note:</strong> 90% (₹900) of this amount will be refunded after you complete one selling and one buying transaction without any reports.</p>
...
<head>
    ...
    <style>
        ...
        .terms-group { display: flex; align-items: center; }
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: left; line-height: 1.6;">
                    <p><strong>eAgri Auction Subscription Terms</strong></p>
                    <ul>
                        <li>The subscription fee of ₹1000 is collected to verify the genuity of users on our platform.</li>
                        <li>Upon completing <strong>one selling transaction</strong> (as a farmer) and <strong>one buying transaction</strong> (as a vendor) through our service without any reports or disputes, 90% of the subscription fee (₹900) will be refunded to your account.</li>
                        <li>The remaining 10% (₹100) will be retained by eAgri Auction as a platform and security development fee to maintain and improve our services.</li>
                        <li>Refunds are processed only after both transactions are successfully completed and verified by our team.</li>
                        <li>Any fraudulent activity or violation of platform policies may result in forfeiture of the refund.</li>
                    </ul>
